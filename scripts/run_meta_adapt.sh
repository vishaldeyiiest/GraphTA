...
model=$2
expt_dir=$3
batch_size=256      ## try 32, 64
adapt=$4
model_group="supc"  ## for logging to WANDB
portion=1
skips=(aux_skip_connection)
#aux_mode="IG MP"
aux_mode="AM EP CP IG MP"  ## or "AM IG MP"

for runseed in $(seq 0 9); do
for skip in ${skips[@]}; do
    ...
    ### for finetuning
    if [[ $adapt == 'ft' ]]; then
    echo "python molecule_finetune.py --input_model_file ../baseline_weights/classification/${model} \
    ...
    echo "python meta_adapt.py --input_model_file ../baseline_weights/classification/${model} \
