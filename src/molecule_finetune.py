...
def eval(model, device, loader, eval_metric):
    model.eval()
    y_true, y_scores = [], []
    total_loss = 0
    ...
    for i in range(y_true.shape[1]):
        ...
        if np.sum(y_true[:, i] == 1) > 0 and np.sum(y_true[:, i] == -1) > 0:
            ...
            roc_list.append(
                eval_metric((y_true[is_valid, i] + 1) / 2, y_scores[is_valid, i])
            )
        else:
    ...
    return sum(roc_list) / len(roc_list), 0, y_true, y_scores, total_loss / len(loader)
...
if __name__ == "__main__":
    ...
    if args.split == "scaffold":
        smiles_list = pd.read_csv(
            dataset_folder + args.dataset + "/processed/smiles.csv", header=None
        )[0].tolist()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            "Loss train: {:.6f}\tval: {:.6f}\ttest: {:.6f}".format(
                train_loss, val_loss, test_loss
            )
        )
        print(
            "ROC train: {:.6f}\tval: {:.6f}\ttest: {:.6f}".format(
        ...
        earlystopping(val_roc)
        if args.dataset in ["hiv", "muv"] and earlystopping.early_stop:
