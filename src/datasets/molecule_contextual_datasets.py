...
class MoleculeContextualDataset(InMemoryDataset):
    def __init__(
        ...
        super(MoleculeContextualDataset, self).__init__(
        ...
        self.atom_vocab_save_path = os.path.join(
            self.root, "processed", self.atom_vocab_file
        )
        self.atom_vocab_label_save_path = os.path.join(
            self.root, "processed", self.atom_vocab_label_file
        )
        if (not os.path.exists(self.atom_vocab_save_path)) or (
            not os.path.exists(self.atom_vocab_label_save_path)
        ):
            if self.molecule_list is None:
                self.molecule_list = self.load_mol_list()
        self.atom_vocab = self.process_contextual_vocabulary(
        ...
        if (not os.path.exists(self.bond_vocab_save_path)) or (
            not os.path.exists(self.bond_vocab_label_save_path)
        ):
            if self.molecule_list is None:
                self.molecule_list = self.load_mol_list()
        self.bond_vocab = self.process_contextual_vocabulary(
