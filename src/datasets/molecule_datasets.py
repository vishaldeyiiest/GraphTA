            ...
            for i in range(len(rdkit_mol_objs)):
                ...
                if rdkit_mol is not None:
                    ...
                    if i in folds[0]:
                        ...
                        fold = torch.tensor([2])
                    data_list.append({"fp_arr": fp_arr, "fold": fold, "id": id, "y": y})
                    data_smiles_list.append(smiles_list[i])
...
def check_columns(df, tasks, N):
    ...
    for task in tasks:
        ...
        pos_ratio = 100.0 * pos_count / (pos_count + neg_count)
        missing_ratio = 100.0 * missing_count / N
        assert pos_count + neg_count + missing_count == N
        if missing_ratio >= 50:
            bad_tasks.append(task)
        print(
            "task {}\t\tpos_ratio: {:.5f}\tmissing ratio: {:.5f}".format(
                task, pos_ratio, missing_ratio
            )
        )
    print(
        "total missing ratio: {:.5f}".format(
            100.0 * total_missing_count / len(tasks) / N
        )
    )
    return bad_tasks
