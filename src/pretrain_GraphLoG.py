...
def intra_NCE_loss(node_reps, node_mask_reps, batch, tau=0.1, epsilon=1e-6):
    ...
    mask = torch.stack(
        [(batch.batch == i).float() for i in batch.batch.tolist()], dim=1
    )
...
def init_proto(args, proto_, proto, proto_state, device, num_iter=20):
    ...
    for iter in range(num_iter):
        for i in range(proto_.shape[0]):
            ...
            proto.data[idx, :] = proto.data[idx, :] * args.decay_ratio + proto_.data[
                i, :
            ] * (1 - args.decay_ratio)
...
def pretrain(args, model, proj, loader, optimizer, device):
    ...
    for step, batch in enumerate(loader):
        ...
        # get node and graph representations
        node_reps = model(batch.x, batch.edge_index, batch.edge_attr)
        node_mask_reps = model(
            batch_mask.x, batch_mask.edge_index, batch_mask.edge_attr
        )
        graph_reps = pool_func(node_reps, batch.batch, mode=args.graph_pooling)
        graph_mask_reps = pool_func(
            node_mask_reps, batch_mask.batch, mode=args.graph_pooling
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--decay", type=float, default=0, help="weight decay in Adam (default: 0)"
    )
    parser.add_argument(
        "--num_layer",
        type=int,
    ...
    parser.add_argument(
        "--output_model_dir", type=str, default="", help="filename to output the model"
    )
    parser.add_argument(
        "--gnn_type", type=str, default="gin", help="GNN model (default: GIN)"
    )
    parser.add_argument(
        "--seed", type=int, default=0, help="Seed for splitting dataset (default: 0)"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    if args.JK == "concat":
        ...
        proto = [
            torch.rand((args.num_proto, args.emb_dim)).to(device)
            for _ in range(args.hierarchy)
        ]
    proto_state = [
        torch.zeros(args.num_proto).to(device) for _ in range(args.hierarchy)
    ]
    proto_connection = []
