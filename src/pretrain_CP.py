...
def do_ContextPred(
    ...
    context_repr = molecule_readout_func(
        overlapped_node_repr, batch.batch_overlapped_context
    )
    ...
    neg_context_repr = torch.cat(
        [
            ...
            for i in range(args.contextpred_neg_samples)
        ],
        dim=0,
    )
...
def train(args, device, loader, optimizer):
    ...
    print(
        "CP Loss: {:.5f}\tCP Acc: {:.5f}\tTime: {:.3f}".format(
...
if __name__ == "__main__":
    ...
    model_param_group = [
        {"params": molecule_substruct_model.parameters(), "lr": args.lr},
    ...
    if not args.output_model_dir == "":
        torch.save(
            molecule_substruct_model.state_dict(), args.output_model_dir + "_model.pth"
        )
