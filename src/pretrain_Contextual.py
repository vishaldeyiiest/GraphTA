...
def train(device, loader, optimizer):
    ...
    for step, batch in enumerate(loader):
        ...
        contextual_loss, contextual_acc = do_Contextual(
            batch, criterion, node_repr, atom_vocab_model
        )
    ...
    print(
        "Contextual Loss: {:.5f}\tContextual Acc: {:.5f}\tTime: {:.5f}".format(
...
if __name__ == "__main__":
    ...
    dataset = MoleculeContextualDataset(
        dataset_folder + args.dataset, dataset=args.dataset
    )
    print(dataset)
    ...
    model_param_group = [
        {"params": molecule_model.parameters()},
