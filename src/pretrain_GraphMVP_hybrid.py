...
def save_model(save_best, epoch=None):
    if not args.output_model_dir == "":
        if save_best:
            ...
            torch.save(
                molecule_model_2D.state_dict(), args.output_model_dir + "_model.pth"
            )
            saver_dict = {
            ...
            torch.save(
                ...
                args.output_model_dir + "_model_final.pth",
            )
            saver_dict = {
                "model": molecule_model_2D.state_dict(),
                "model_3D": molecule_model_3D.state_dict(),
            ...
            torch.save(
                molecule_model_2D.state_dict(),
                args.output_model_dir + "_model_{}.pth".format(epoch),
            )
            saver_dict = {
                "model": molecule_model_2D.state_dict(),
                "model_3D": molecule_model_3D.state_dict(),
            ...
            torch.save(
                saver_dict,
                args.output_model_dir + "_model_complete_{}.pth".format(epoch),
            )
...
def train_no_aug(args, device, loader, optimizer):
    ...
    temp_loss = (
        ...
        + args.alpha_3 * SSL_2D_loss_accum
    )
...
if __name__ == "__main__":
    ...
    if args.SSL_2D_mode in ["GraphCL", "JOAO", "JOAOv2"]:
        dataset = MoleculeGraphCLMaskingDataset(
            data_root, dataset=args.dataset, mask_ratio=args.SSL_masking_ratio
        )
    ...
    if args.SSL_2D_mode == "EP":
        ...
        loader = DataLoaderSubstructContext3D(
            dataset,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
    ...
    for SSL_2D_support_model in SSL_2D_support_model_list:
        model_param_group.append(
            {
                "params": SSL_2D_support_model.parameters(),
