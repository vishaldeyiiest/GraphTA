from batch import (BatchAE, BatchMasking, BatchSubstructContext,
                   BatchSubstructContext3D, BatchHybrid)
...
class DataLoaderHybrid(DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderHybrid, self).__init__(
            ...
            shuffle,
            collate_fn=
            lambda data_list: [BatchHybrid.from_data_list([_[0] for _ in data_list]),
                               Batch.from_data_list([_[1] for _ in data_list]),
                               Batch.from_data_list([_[2] for _ in data_list])] \
                if isinstance(data_list[0], tuple) or isinstance(data_list[0], list) else BatchHybrid.from_data_list(data_list),
            **kwargs)
