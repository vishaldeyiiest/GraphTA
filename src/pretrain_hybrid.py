...
from models import GNN, AutoEncoder, Discriminator, SchNet, VariationalAutoEncoder
...
def compute_loss(
    args, batch, model, aux_2D_support_model_list, criterion, aux_model=None
):
    if args.adapt == "uncert":
        node_repr, logsigma = model.molecule_model(
    ...
    aux_2D_loss_dict, aux_2D_acc_dict = get_aux_2D_loss(
        args,
        batch,
        node_repr,
...
def get_aux_2D_loss(
    ...
    aux_2D_loss, aux_2D_acc = dict.fromkeys(args.aux_2D_mode, 0), dict.fromkeys(
        args.aux_2D_mode, 0
    )
...
def train_no_aug(args, device, loader, optimizer):
    ...
    for batch in l:
        ...
        if args.aux_2D_mode:
            ...
            aux_2D_loss = torch.mean(
                torch.stack(list(aux_2D_loss_dict.values()))
            ...
            for mode in args.aux_2D_mode:
                ...
                aux_2D_loss_accum_dict[mode] += (
                    aux_2D_loss_dict[mode].detach().cpu().item()
                )
        else:
            raise Exception
    ...
    temp_loss = (
        args.alpha_1 * CL_loss_accum
        + args.alpha_2 * AE_loss_accum
        + args.alpha_3 * aux_2D_loss_accum
    )
    ...
    print_str = (
        f"CL Loss: {CL_loss_accum:.5f}  CL Acc: {CL_acc_accum:.5f}\t"
...
def train_with_aug(args, device, loader, optimizer):
    ...
    if "JOAO" in args.aux_2D_mode:
        aug_prob = update_augmentation_probability_JOAO(
            loader=loader,
            molecule_model_2D=molecule_model_2D,
            projection_head=aux_2D_support_model_list["JOAO"],
            molecule_readout_func=molecule_readout_func,
...
if __name__ == "__main__":
    ...
    data_root = (
        "../../datasets/{}/".format("GEOM_3D_nmol50000_nconf5_nupper1000")
    ...
    if set(args.aux_2D_mode).intersection(set(["GraphCL", "JOAO", "JOAOv2"])):
        ...
        dataset = Molecule3DHybridDataset(
            data_root,
            dataset=args.dataset,
            transform=compose_transform,
            mask_ratio=args.SSL_masking_ratio,
    ...
    # set up parameters
    model_param_group = [
        {"params": molecule_model_2D.parameters(), "lr": args.lr * args.gnn_lr_scale}
    ]
    ...
    for mode, aux_2D_support_model in aux_2D_support_model_list.items():
        if mode != "EP":
            model_param_group.append(
                {
                    "params": aux_2D_support_model.parameters(),
